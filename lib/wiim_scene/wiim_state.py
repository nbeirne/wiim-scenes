...
class WiimState:
    ...
    def apply_scene(self, scene):
        ...
        if "output" in scene:
            ...
            if "airplay" in self.state["output"] and "airplay" in scene["output"]:
                new_state["output"]["airplay"] = self.apply_airplay_queries(self.state["output"]["airplay"], scene["output"]["airplay"])

            elif "airplay" in scene["output"]:
                new_state["output"]["airplay"] = scene["output"]["airplay"]

        return WiimState(new_state)
    ...
    def does_scene_match(self, scene, match_volume=False):
        ...
        if "output" in scene:
            ...
            if self.state["output"]["mode"] == "airplay" and "airplay" in self.state["output"] and "airplay" in scene["output"]:
                if not self.does_airplay_devices_match_queries(self.state["output"]["airplay"], scene["output"]["airplay"], match_volume):
                    return False
    ...
    def apply_airplay_queries(self, airplay_devices, airplay_queries):
        airplay_devices = deepcopy(airplay_devices)
        new_airplay_devices = []
        for airplay_device in airplay_devices:
            for query in airplay_queries:
                if self.does_airplay_device_match_query(query, airplay_device):
                    airplay_device = {
                        **airplay_device,
                        **query
                    }
...
# querying
    ...
    def does_airplay_device_match_query(self, query, airplay_device):
    ...
    def does_airplay_devices_match_queries(self, airplay_devices, airplay_queries, match_volume):
        devices = {}
